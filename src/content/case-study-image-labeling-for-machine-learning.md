...
```javascript
<Stage onMouseDown={startDrawing} onMouseMove={addNewPoints} onMouseUp={finishDrawing}>
  <BaseImage />
...
```javascript
<Line points={region.points} fill={region.color} opacity={0.7} closed />
```
...
const Regions = ({ regions }) => {
  ...
  return (
    <Layer ref={layerRef}>
      {regions.map((region) => {
        return <Line points={region.points} fill={region.color} closed />;
      })}
    </Layer>
  );
};
